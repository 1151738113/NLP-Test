...
/**
 * Created by wei.wang on 2018/8/16 0016.
 * 垃圾邮件过滤，基于贝叶斯分类算法
 * <p>
 * 垃圾邮件分类实现逻辑
 * <p>
 * 计算正常邮件的idf，计算垃圾邮件的idf。（用分词器）
 * <p>
 * 对测试数据分词。
 * 通过P(A|B) = P(A)/P(AB)
 */
public class SpamMailDetection {
     ...
     */
    private Map<String, Double> createSpamProbabilityMap(Map<String, Double> spammap,
    ...
    //读取停用词
    private List<String> getStopDic() {
        List<String> list = Lists.newArrayList();
        try {
            BufferedReader br = new BufferedReader(new InputStreamReader(
                    new FileInputStream(new File("F:\\ww\\bayes\\BayesSpam\\data\\stop.txt")), "gbk"));
            String tmp = "";
            while ((tmp = br.readLine()) != null) {
                list.add(tmp);
            }
